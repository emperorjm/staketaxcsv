...
from staketaxcsv.arch.make_tx import make_arch_unknown_tx, make_arch_unknown_tx_with_transfer
import logging
...
def handle_unknown_detect_transfers(exporter, txinfo, msginfo):
    ...
    if len(transfers_in) == 0 and len(transfers_out) == 0:
        ...
        row = make_arch_unknown_tx_with_transfer(
        ...
        for sent_amount, sent_currency in transfers_net_out:
            rows.append(
                make_arch_unknown_tx_with_transfer(txinfo, msginfo, sent_amount, sent_currency, "", ""))
        for received_amount, received_currency in transfers_net_in:
            rows.append(
                make_arch_unknown_tx_with_transfer(txinfo, msginfo, "", "", received_amount, received_currency))
        util_arch._ingest_rows(exporter, rows, "")
